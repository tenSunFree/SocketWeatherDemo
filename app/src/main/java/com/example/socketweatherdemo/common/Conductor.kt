...
abstract class ScopedController<VM : Any, P : Any>(args: Bundle? = null) : Controller(args) {
    ...
    final override fun onCreateView(
        ...
        val vm: VM =
            viewModel ?: onCreateViewModel(view.context.applicationContext).also { viewModel = it }
    ...
    final override fun onAttach(view: View) {
        ...
        viewScope = scope
        val presenterToAttach =
            requireNotNull(presenter) { "presenter shouldn't be null in onAttach()." }
    ...
    final override fun onDetach(view: View) {
        ...
        val presenterToDetach =
            requireNotNull(presenter) { "presenter shouldn't be null in onDetach()." }
    ...
    final override fun onDestroyView(view: View) {
        val currentPresenter =
            requireNotNull(presenter) { "presenter shouldn't be null in onDestroyView()." }
